...
// Sandbox環境変数を安全にチェック
const sandboxConfig = (typeof window !== 'undefined' && window.__firebase_config) 
  ? JSON.parse(window.__firebase_config) 
  : null;
if (sandboxConfig) firebaseConfig = sandboxConfig;
...
// appIdの定義（スラッシュ対策）
const rawAppId = (typeof window !== 'undefined' && window.__app_id) ? window.__app_id : 'shift-flow-production-v1';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
      try {
        ...
        if (err.code === 'auth/api-key-not-valid' || err.code === 'auth/invalid-api-key') setConfigError(true);
      }
    };
    initAuth();
  ...
  useEffect(() => {
    ...
    const handleErr = (err) => { if (err.code === 'permission-denied') setPermissionError(true); };

    const unsubPass = onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'auth'), (snap) => {
      const d = {}; snap.forEach(doc => d[doc.id] = doc.data().password);
      setStoredPasswords(d); setIsDataLoaded(true);
    }, handleErr);
    ...
    const unsubAssign = onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'assignments'), (snap) => {
      const d = {}; snap.forEach(doc => d[doc.id] = doc.data());
      setAssignments(d);
    }, handleErr);

    const unsubTasks = onSnapshot(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'tasks'), (snap) => {
      if (snap.exists()) {
        const list = snap.data().list || INITIAL_TASKS;
        setTasks(list); if (!selectedTask && list.length > 0) setSelectedTask(list[0]);
      }
    }, handleErr);
  ...
  const handleLoginSubmit = async (e) => {
    ...
    if (authStep === 'setup') {
      ...
      setCurrentUserProfile(profile); setUserRole(tid === 'manager' ? 'manager' : 'staff');
  ...
  const saveAvail = async (sid, d, payload) => {
    await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'availability', getCellKey(sid, d)), { ...(availability[getCellKey(sid, d)] || {}), ...payload });
    setEditingCell(null);
  };

  const saveAssign = async (sid, d, taskName) => {
    const avail = availability[getCellKey(sid, d)];
    ...
    await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'assignments', getCellKey(sid, d)), { task: taskName });
  };
  ...
  // --- ビュー ---
  if (configError) return <div className="h-screen flex items-center justify-center bg-slate-900 text-white p-10">Firebase Config Error</div>;
  if (permissionError) return <div className="h-screen flex items-center justify-center bg-slate-900 text-white p-10">Firestore Permission Denied</div>;
  ...
  if (!currentUserProfile) return (
    <div className="h-screen bg-slate-950 flex items-center justify-center p-4 font-sans">
      <div className="max-w-md w-full bg-white rounded-[2.5rem] shadow-2xl p-10">
        {!isDataLoaded ? (
          <div className="flex flex-col items-center py-10 gap-4"><Loader2 className="animate-spin text-indigo-600" size={40}/><p className="font-black text-slate-400 text-xs">Connecting...</p></div>
        ) : (
          <form onSubmit={handleLoginSubmit}>
            ...
            {errorMessage && <div className="mt-4 text-rose-500 text-xs font-black flex items-center gap-1"><AlertTriangle size={14}/>{errorMessage}</div>}
            <button type="submit" className="w-full py-5 rounded-2xl bg-indigo-600 text-white font-black mt-8 hover:bg-indigo-700 shadow-xl">ログイン</button>
          </form>
        )}
      </div>
    </div>
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 text-slate-800 font-sans select-none overflow-hidden" onMouseUp={handleDragEnd}>
      <header className="bg-white border-b px-8 py-4 flex items-center justify-between shadow-sm z-50 shrink-0">
        <div className="flex items-center gap-5">
          <div className="bg-indigo-600 p-2 rounded-xl"><Calendar className="text-white" size={20} /></div>
          <div><h1 className="text-lg font-black leading-none text-slate-900">ShiftFlow Pro</h1><div className="flex items-center gap-2 mt-1"><span className="text-[9px] font-black text-indigo-500 uppercase tracking-widest">{userRole}</span><div className="flex items-center gap-2 bg-slate-100 px-2 py-0.5 rounded-full border border-slate-200 text-slate-600"><button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth()-1, 1))}><ChevronLeft size={14}/></button><span className="text-[10px] font-black min-w-[70px] text-center">{currentMonth.getFullYear()}年 {currentMonth.getMonth()+1}月</span><button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth()+1, 1))}><ChevronRight size={14}/></button></div></div></div>
        </div>
        <div className="flex items-center gap-4"><div className="px-4 py-2 bg-slate-50 rounded-xl border border-slate-100 flex items-center gap-3"><div className={`w-8 h-8 rounded-lg flex items-center justify-center font-black text-sm shadow-sm ${userRole === 'manager' ? 'bg-slate-800 text-white' : 'bg-emerald-500 text-white'}`}>{currentUserProfile.name[0]}</div><span className="text-sm font-black text-slate-800">{currentUserProfile.name}</span></div><button onClick={() => window.location.reload()} className="p-2 text-slate-300 hover:text-rose-500"><LogOut size={22} /></button></div>
      </header>
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 overflow-auto p-8 bg-slate-50/50">
          <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 min-w-max overflow-hidden relative">
            <table className="w-full border-collapse"><thead className="bg-slate-50 border-b"><tr><th className="sticky left-0 z-40 bg-slate-50 p-8 text-left border-r w-64 text-[10px] text-slate-400 font-black uppercase tracking-widest shadow-sm">Attendance</th>{dates.map(d => { const dt = new Date(d); return (<th key={d} className="p-6 border-r min-w-[220px] text-center text-slate-800"><div className="text-[10px] text-slate-400 uppercase mb-1 font-black">{dt.toLocaleDateString('ja-JP', { weekday: 'short' })}</div><div className={`text-2xl font-black ${dt.getDay() === 0 ? 'text-rose-500' : dt.getDay() === 6 ? 'text-sky-500' : 'text-slate-800'}`}>{dt.getDate()}</div></th>); })}</tr></thead>
              <tbody>{STAFF_LIST.map(staff => { const isMy = currentUserProfile?.id === staff.id; if (userRole === 'staff' && !isMy) return null; return (<tr key={staff.id} className="border-b border-slate-50 group text-slate-800">
                <td className="sticky left-0 z-30 bg-white p-6 border-r group-hover:bg-slate-50 transition-colors shadow-sm text-slate-800"><div className="flex items-center gap-4"><div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-lg ${isMy ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-100 text-slate-400'}`}>{staff.name[0]}</div><div className="flex flex-col text-slate-800"><span className="text-sm font-black text-slate-800">{staff.name}</span>{isMy && <span className="text-[8px] font-black text-indigo-500 uppercase tracking-tighter">Owner</span>}</div></div></td>
                {dates.map(d => {
